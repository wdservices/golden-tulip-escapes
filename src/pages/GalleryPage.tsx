...
// Bento-style span pattern for editorial variety
const spanPattern = [
...
export default function GalleryPage() {
  ...
  return (
    <div className="min-h-screen bg-[#f7f9fc] text-slate-900 relative overflow-x-hidden">
      {/* Ambient background gradients (brand: blue + gold) */}
      <div aria-hidden className="pointer-events-none absolute inset-0 overflow-hidden">
        <div className="absolute -top-40 -left-40 w-[520px] h-[520px] rounded-full bg-gradient-to-br from-blue-400/25 via-blue-300/10 to-transparent blur-3xl" />
        <div className="absolute top-1/3 -right-40 w-[600px] h-[600px] rounded-full bg-gradient-to-br from-amber-300/30 via-yellow-200/10 to-transparent blur-3xl" />
        <div className="absolute bottom-0 left-1/3 w-[420px] h-[420px] rounded-full bg-gradient-to-tr from-blue-500/15 to-transparent blur-3xl" />
      </div>

      {/* Header */}
      <header className="sticky top-0 z-40 backdrop-blur-xl bg-white/70 shadow-[0_1px_0_rgba(15,23,42,0.06),0_10px_30px_-20px_rgba(15,23,42,0.15)]">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4 min-w-0">
            <Link
              to="/"
              className="inline-flex items-center gap-2 px-3 py-2 rounded-full bg-white text-slate-700 hover:text-blue-700 shadow-[0_4px_14px_rgba(15,23,42,0.08)] hover:shadow-[0_8px_22px_rgba(37,99,235,0.25)] transition-all duration-300"
            >
              <ArrowLeft className="h-4 w-4" />
              <span className="text-sm font-medium">Back</span>
            </Link>
            <div className="flex items-center gap-3 min-w-0">
              <span className="hidden sm:inline-flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-br from-blue-600 to-blue-500 text-white shadow-[0_10px_25px_-8px_rgba(37,99,235,0.6)]">
                <Camera className="h-4 w-4" />
              </span>
              <div className="min-w-0">
                <p className="text-[11px] uppercase tracking-[0.2em] text-blue-700/70 font-semibold">Golden Tulip · Gallery</p>
                <h1 className="text-xl md:text-2xl font-serif font-bold leading-tight truncate">
                  {selectedCategory ? (
                    <span className="bg-gradient-to-r from-blue-700 via-blue-600 to-amber-500 bg-clip-text text-transparent">
                      {selectedCategory}
                    </span>
                  ) : (
                    <span className="bg-gradient-to-r from-slate-900 via-blue-800 to-amber-600 bg-clip-text text-transparent">
                      Moments in Frame
                    </span>
                  )}
                </h1>
          ...
          {selectedCategory && (
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold text-white bg-gradient-to-r from-blue-600 to-blue-500 shadow-[0_10px_25px_-8px_rgba(37,99,235,0.55)] hover:shadow-[0_14px_30px_-8px_rgba(37,99,235,0.75)] hover:-translate-y-0.5 transition-all duration-300"
            >
              All Categories
      ...
      <main className="container mx-auto px-4 py-10 md:py-16 relative">
        {!selectedCategory ? (
          <>
            {/* Hero intro */}
            <motion.section
            ...
            >
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white shadow-[0_6px_18px_rgba(15,23,42,0.08)] text-xs font-semibold text-blue-700 mb-6">
              ...
              <h2 className="font-serif text-4xl md:text-6xl leading-[1.05] tracking-tight">
                ...
                <span className="relative inline-block">
                  ...
                  <span className="absolute -bottom-1 left-0 right-0 h-1 bg-gradient-to-r from-blue-600 to-amber-400 rounded-full opacity-70" />
                </span>
              ...
              {categories.map(({ key, thumb, blurb }, i) => {
                ...
                return (
                  <motion.button
                    ...
                    className={`group relative overflow-hidden rounded-3xl text-left bg-white shadow-[0_10px_30px_-12px_rgba(15,23,42,0.25)] hover:shadow-[0_28px_60px_-15px_rgba(37,99,235,0.45)] transition-all duration-500 will-change-transform hover:-translate-y-1 ${spanPattern[i % spanPattern.length]}`}
                  >
                    <img
                      src={thumb}
                      alt={key}
                    ...
                    />
                    {/* Blue-gold color wash on hover */}
                    <div className="absolute inset-0 bg-gradient-to-tr from-blue-900/70 via-blue-800/25 to-transparent" />
                    <div className="absolute inset-0 bg-gradient-to-tr from-transparent via-transparent to-amber-400/0 group-hover:to-amber-400/40 transition-all duration-700" />
                    {/* Golden shine sweep */}
                    <div className="pointer-events-none absolute -inset-x-8 -top-1/2 h-[220%] rotate-12 bg-gradient-to-r from-transparent via-white/40 to-transparent opacity-0 group-hover:opacity-100 translate-x-[-120%] group-hover:translate-x-[120%] transition-all duration-[1400ms] ease-out" />

                    {/* Content */}
                    <div className="absolute inset-0 p-5 md:p-6 flex flex-col justify-between">
                      <div className="flex items-start justify-between">
                        <span className="inline-flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-[0.18em] text-white/95 bg-white/15 backdrop-blur-md px-2.5 py-1 rounded-full shadow-[0_4px_12px_rgba(0,0,0,0.15)]">
                          <span className="h-1.5 w-1.5 rounded-full bg-amber-400 shadow-[0_0_10px_rgba(251,191,36,0.9)]" />
                          {count} photo{count !== 1 ? "s" : ""}
                        </span>
                        <span className="h-9 w-9 rounded-full bg-white/95 text-blue-700 flex items-center justify-center shadow-[0_8px_20px_rgba(0,0,0,0.2)] transition-all duration-500 group-hover:bg-amber-400 group-hover:text-white group-hover:rotate-45">
                          <ArrowUpRight className="h-4 w-4" />
                        </span>
                      </div>
                      <div className="transform transition-all duration-500 translate-y-1 group-hover:translate-y-0">
                        ...
                        <p className="mt-1 text-sm text-white/85 opacity-0 max-h-0 group-hover:opacity-100 group-hover:max-h-20 transition-all duration-500 overflow-hidden">
                          {blurb}
                        </p>
                        <div className="mt-3 h-[3px] w-10 rounded-full bg-gradient-to-r from-amber-400 to-blue-400 shadow-[0_0_12px_rgba(251,191,36,0.6)] transition-all duration-500 group-hover:w-24" />
                      </div>
                    </div>
                  </motion.button>
                );
          ...
          <>
            ...
            >
              <div>
                <p className="text-[11px] uppercase tracking-[0.24em] font-bold text-amber-600">Collection</p>
                ...
                <p className="text-sm text-slate-500 mt-1">{images.length} photograph{images.length !== 1 ? "s" : ""}</p>
              </div>
            </motion.div>
            ...
            <div className="columns-1 sm:columns-2 lg:columns-3 xl:columns-4 gap-4 md:gap-5 [column-fill:_balance]">
              {images.map((src, index) => (
                <motion.button
                  ...
                  className="group relative mb-4 md:mb-5 w-full block break-inside-avoid overflow-hidden rounded-2xl bg-white shadow-[0_8px_24px_-10px_rgba(15,23,42,0.25)] hover:shadow-[0_24px_50px_-15px_rgba(37,99,235,0.45)] transition-all duration-500 hover:-translate-y-1"
                >
                  <img
                    src={src}
                    ...
                    onError={(e) => { (e.target as HTMLImageElement).src = fallback; }}
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-blue-950/70 via-blue-900/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                  <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between opacity-0 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500">
                    <span className="text-[11px] font-bold uppercase tracking-[0.2em] text-white/95 bg-white/10 backdrop-blur-md px-2.5 py-1 rounded-full">
                      #{String(index + 1).padStart(2, "0")}
                    </span>
                    <span className="h-8 w-8 rounded-full bg-amber-400 text-white flex items-center justify-center shadow-[0_8px_20px_rgba(251,191,36,0.55)]">
                      <ArrowUpRight className="h-4 w-4" />
                    </span>
                  </div>
                </motion.button>
        ...
        {lightboxIndex !== null && (
          ...
          >
            <button
              ...
              className="absolute top-5 right-5 h-11 w-11 rounded-full bg-white/10 backdrop-blur-md text-white flex items-center justify-center shadow-[0_10px_30px_rgba(0,0,0,0.4)] hover:bg-amber-400 hover:text-white hover:shadow-[0_10px_30px_rgba(251,191,36,0.5)] transition-all duration-300 hover:rotate-90"
              aria-label="Close"
            >
              <X className="h-5 w-5" />
            </button>
            ...
            <button
              onClick={handlePrev}
              className="absolute left-4 md:left-8 top-1/2 -translate-y-1/2 h-12 w-12 rounded-full bg-white/10 backdrop-blur-md text-white flex items-center justify-center shadow-[0_10px_30px_rgba(0,0,0,0.4)] hover:bg-blue-500 hover:shadow-[0_10px_30px_rgba(37,99,235,0.5)] transition-all duration-300 hover:-translate-x-1"
              aria-label="Previous"
            >
            ...
            <button
              onClick={handleNext}
              className="absolute right-4 md:right-8 top-1/2 -translate-y-1/2 h-12 w-12 rounded-full bg-white/10 backdrop-blur-md text-white flex items-center justify-center shadow-[0_10px_30px_rgba(0,0,0,0.4)] hover:bg-blue-500 hover:shadow-[0_10px_30px_rgba(37,99,235,0.5)] transition-all duration-300 hover:translate-x-1"
              aria-label="Next"
            >
            ...
            >
              <div className="rounded-3xl overflow-hidden shadow-[0_40px_100px_-20px_rgba(37,99,235,0.5)] ring-1 ring-white/10">
                <img
              ...
              <div className="mt-5 flex items-center justify-center gap-3">
                <span className="px-4 py-1.5 rounded-full bg-white/10 backdrop-blur-md text-white text-sm font-medium shadow-[0_6px_20px_rgba(0,0,0,0.3)]">
                  <span className="text-amber-300 font-bold">{lightboxIndex + 1}</span>
                  <span className="opacity-60"> / {images.length}</span>
                </span>
                <span className="text-xs text-white/60 uppercase tracking-[0.25em]">{selectedCategory}</span>
              </div>
            </motion.div>
          </motion.div>
        )}
